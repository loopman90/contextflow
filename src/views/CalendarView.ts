...
export class CalendarView extends ItemView {
  ...
  private render(): void {
    ...
    const exportButton = header.createEl("button", { text: "ICS export" }); exportButton.addEventListener("click", () => { const link = document.body.createEl("a"); link.href = URL.createObjectURL(new Blob([eventsToIcs(this.plugin.settings.calendarEvents)], { type: "text/calendar" })); link.download = "contextflow-calendar.ics"; link.click(); link.remove(); });
    const importButton = header.createEl("button", { text: "ICS import" }); importButton.addEventListener("click", () => { const input = root.createEl("input", { attr: { type: "file", accept: ".ics,text/calendar" } }); input.addEventListener("change", () => { const file = input.files?.[0]; if (!file) return; const reader = new FileReader(); reader.addEventListener("load", () => { if (typeof reader.result === "string") { this.plugin.settings.calendarEvents.push(...parseIcs(reader.result)); void this.plugin.saveSettings(); this.render(); } input.remove(); }); reader.readAsText(file); }); input.click(); });
