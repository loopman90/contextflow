...
export class ContextFlowSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "ContextFlow" });
    ...
    new Setting(containerEl).setName(t("maxResults", language)).addSlider((slider) => slider.setLimits(3, 30, 1).setValue(this.plugin.settings.slashMaxResults).setDynamicTooltip().onChange(async (value) => { this.plugin.settings.slashMaxResults = value; await this.plugin.saveSettings(); }));
    ...
    new Setting(containerEl).setName(t("recentActions", language)).setDesc(language === "en" ? "Number of action IDs stored locally." : "Aantal actie-ID's dat lokaal wordt bewaard.").addSlider((slider) => slider.setLimits(0, 20, 1).setValue(this.plugin.settings.maxRecentActions).setDynamicTooltip().onChange(async (value) => { this.plugin.settings.maxRecentActions = value; this.plugin.settings.recentActionIds = this.plugin.settings.recentActionIds.slice(0, value); await this.plugin.saveSettings(); }));
